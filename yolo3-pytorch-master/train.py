...
def fit_one_epoch(
    net, yolo_loss, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda
):
    total_loss = 0
    val_loss = 0
    ...
    with tqdm(
        ...
        for iteration, batch in enumerate(gen):
            ...
            pbar.set_postfix(
                **{"total_loss": total_loss / (iteration + 1), "lr": get_lr(optimizer)}
            )
            pbar.update(1)
...
if __name__ == "__main__":
    ...
    if True:
        ...
        train_dataset = YoloDataset(
            lines[:num_train], (input_shape[0], input_shape[1]), True
        )
        val_dataset = YoloDataset(
            lines[num_train:], (input_shape[0], input_shape[1]), False
        )
        gen = DataLoader(
            train_dataset,
            shuffle=True,
