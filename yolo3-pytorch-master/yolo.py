    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            batch_detections = non_max_suppression(
                output, self.num_classes, conf_thres=self.confidence, nms_thres=self.iou
            )
            ...
            # ---------------------------------------------------------#
            top_index = (
                    batch_detections[:, 4] * batch_detections[:, 5] > self.confidence
            )
    ...
    def get_FPS(self, image, test_interval):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                top_conf = (
                        batch_detections[top_index, 4] * batch_detections[top_index, 5]
                )
                top_label = np.array(batch_detections[top_index, -1], np.int32)
                top_bboxes = np.array(batch_detections[top_index, :4])
                top_xmin, top_ymin, top_xmax, top_ymax = (
