...
class DecodeBox(nn.Module):
    ...
    def forward(self, input):
        ...
        prediction = (
            input.view(
                batch_size, self.num_anchors, self.bbox_attrs, input_height, input_width
            )
            .permute(0, 1, 3, 4, 2)
            .contiguous()
        )
        ...
        grid_y = (
            torch.linspace(0, input_height - 1, input_height).repeat(input_width, 1).t().repeat(batch_size * self.num_anchors, 1, 1).view(y.shape).type(FloatTensor)
        )
...
def yolo_correct_boxes(top, left, bottom, right, input_shape, image_shape):
    ...
    box_yx = (
        np.concatenate(((top + bottom) / 2, (left + right) / 2), axis=-1) / input_shape
    )
